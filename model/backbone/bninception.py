...
class BNInception(nn.Module):
    ...
    def __init__(self, weight_path='weights_models', pretrained=True):
        ...
        self.model = bninception(num_classes=1000, pretrained="imagenet" if pretrained else None, weight_path=weight_path)
        
        self.model.global_pool = GMP_and_GAP()
...
def bninception(num_classes=1000, pretrained="imagenet", weight_path='weights_models'):
    r"""BNInception model architecture from <https://arxiv.org/pdf/1502.03167.pdf>`_ paper.
    """
    model = BNInceptionImageNet(num_classes=num_classes)
    if pretrained is not None:
        ...
        weight = torch.load(os.path.join(weight_path, 'bn_inception-52deb4733.pth'))
